...
import { validate } from './TypeCheck'
...
export class TapAssert {
  ...
  check (message: string, pass: boolean, failReason: string): boolean {
    let expectation = 'ok'
    if (!pass) {
      expectation = failReason
    }
    if (this._enableThrow) this._t.ok(pass, message + ': ' + expectation)
    return pass
  }
  ...
  isType (item: any, type: string, message: string): boolean {
    ...
    return this.check(message, typeof item === type, `Expected type to be '${type}'. Got '${typeof item}'`)
  }
  ...
  isNotType (item: any, type: string, message: string): boolean {
    /* eslint valid-typeof: "off" */
    return this.check(message, typeof item !== type, `Expected type to not be '${type}'`)
  }
  ...
  isEqual (item: any, equivalent: any, message: string): boolean {
    ...
    return this.check(message, item === equivalent, `Expected to be '${eqstr}'. Got '${itstr}' (${typeof item})`)
  }
  ...
  isNotEqual (item: any, equivalent: any, message: string): boolean {
    const itstr: string = item.toString()
    return this.check(message, item !== equivalent, `Expected value not to equal item '${itstr}' (${typeof item})`)
  }
  ...
  isTruthy (item: any, message: string): boolean {
    const itstr: string = item.toString()
    const truthy: boolean = item
    return this.check(message, truthy, `Expected value to be "truthy". Got "${itstr}"`)
  }
  ...
  isNotTruthy (item: any, message: string): boolean {
    const itstr: string = item.toString()
    const truthy: boolean = item
    return this.check(message, !truthy, `Expected value to be "not truthy". Got "${itstr}"`)
  }
  ...
  isTrue (item: any, message: string): boolean {
    const itstr: string = item.toString()
    return this.check(message, item === true, `Expected value to be true. Got "${itstr}"`)
  }
  ...
  isFalse (item: any, message: string): boolean {
    const itstr: string = item.toString()
    return this.check(message, item === false, `Expected value to be false. Got "${itstr}"`)
  }

  private checkEmpty (item: any, not: boolean): { empty: boolean, reason: string } {
    let empty = false
    let reason = ''
    if (Array.isArray(item)) {
      empty = item.length === 0
      reason = not ? 'Array is empty' : 'Array has elements'
    } else if (typeof item === 'object') {
      empty = Object.getOwnPropertyNames(item).length === 0
      reason = not ? 'Object is empty' : 'Object has properties'
    } else if (typeof item === 'string') {
      empty = item.length === 0
      reason = not ? 'string is empty' : 'string has length'
    }
    return { empty, reason }
  }

  private checkContains (item: any, target: any, not: boolean): { contains: boolean, reason: string } {
    let contains = false
    let reason = ''
    const tstr: string = target.toString()
    if (Array.isArray(item)) {
      contains = item.includes(target)
      reason = not ? 'Array contains ' + tstr : 'Array does not contain ' + tstr
    } else if (typeof item === 'object') {
      contains = item[target] !== undefined
      reason = not ? 'Object has property ' + tstr : 'Object does not have property ' + tstr
    } else if (typeof item === 'string') {
      contains = item.includes(target)
      reason = not ? `string contains '${tstr}'` : `string does not contain '${tstr}'`
    }
  ...
  isEmpty (item: any, message: string): boolean {
    const { empty, reason } = this.checkEmpty(item, false)
    return this.check(message, empty, reason)
  }
  ...
  isNotEmpty (item: any, message: string): boolean {
    const { empty, reason } = this.checkEmpty(item, true)
    return this.check(message, !empty, reason)
  }
  ...
  contains (item: any, target: any, message: string): boolean {
    ...
    return this.check(message, contains, reason)
  }
  ...
  doesNotContain (item: any, target: any, message: string): boolean {
    const { contains, reason } = this.checkContains(item, target, true)
    return this.check(message, !contains, reason)
  }
  ...
  meetsConstraints (item: any, constraints: string, message: string): boolean {
    const reason: string = validate(item, constraints)
    const pass = reason === ''
    return this.check(message, pass, reason)
  }
}
